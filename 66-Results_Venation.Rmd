...
stat.test <- aov(lm(log10(MajVLA)~Forest, data=Metradica)) %>% tukey_hsd() 
...
G
stat.test.LA <- aov(lm(log10(LA)~Forest, data=Metradica)) %>% tukey_hsd() 

H <- ggboxplot(data = Metradica, x = "Forest", y = "LA", fill = "Forest") +
  scale_fill_manual(
    values = list(
      Bafog = "#F4C71C",
      Paracou = "#66FF61", 
  ...
  theme(
    ...
    axis.text.x=element_blank(),
    axis.title.y = element_text(size = 15)) + 
  stat_pvalue_manual(stat.test.LA, label = "p.adj.signif",
                       y.position = c(285,300,320))

H

library(ggpubr)

ggarrange(H, G, ncol=2, nrow=1, common.legend = TRUE, legend="bottom", labels = c('A', 'B'))
```

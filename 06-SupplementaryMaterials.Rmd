...
##Figure S8:  Crown position and dawkins 
```{r}
...
complementary_info <- read.csv("C://Users/marion.boisseaux/Dropbox/Mon PC (Jaboty20)/Documents/METRADICA/METRADICAproject/Dataset/Metradica_all_fieldsheet.csv") %>% select(Code, TreeDawkins, BranchDawkins, CrownPosition) #already have DBH
...
Phos.plot <- data_with_complementary_info %>%
  ...
  ggplot(aes(as.factor(TreeDawkins), Phosphorous)) + 
   geom_jitter(alpha=0.2)+
  geom_boxplot(aes(fill = factor(..middle..)), show.legend = FALSE, alpha=0.6) +
  labs(x="Tree Dawkins", y="Phosphorous") +
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
...
SD.plot <- data_with_complementary_info %>%
  ...
  geom_jitter(alpha=0.2)+
 geom_boxplot(aes(fill = factor(..middle..)), show.legend = FALSE, alpha=0.6) +
    labs(x="Tree Dawkins", y="Stomatal density") +
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +

...
## Figure S6: Trait contributions to first four principal component axes among leaf traits. 
...
##Figure S7: PCA with the third axis.

See Analysis script: trait covariation section.

## Figure S8: Spearman correlation coefficients among leaf traits. 
...
## Figure S9: Correlation between stomatal density and minimal leaf conductance. 
...
##Figure S10: Dawkins index

It's an image from Alder et al 1992.

##Figure S11:  Crown position and dawkins 
```{r}
...
complementary_info <- read.csv("C://Users/marion.boisseaux/Dropbox/Mon PC (Jaboty20)/Documents/METRADICA/METRADICAproject/Dataset/Metradica_all_fieldsheet.csv") %>% dplyr::select(Code, TreeDawkins, BranchDawkins, CrownPosition) #already have DBH
...
gmin.plot <- data_with_complementary_info %>%
  ...
  geom_text(data = Tk.gmin, aes(x = TreeDawkins, y = quant, label = cld), size = 6, vjust=-1, hjust =-1) +
...
TLP.plot <- data_with_complementary_info %>%
  ...
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  annotate(geom="text", x= 2, y=-0.25, label="Anova, pvalue = 0.4", size = 4) +
  scale_fill_brewer(palette = "Blues")
...
MajVLA.plot <- data_with_complementary_info %>%
  ...
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_text(data = Tk.MajVLA, aes(x = TreeDawkins, y = quant, label = cld), size =6, vjust=-1, hjust =-1) +
  scale_fill_brewer(palette = "Blues")
...
N.plot <- data_with_complementary_info %>%
  ...
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_text(data = Tk.N, aes(x = TreeDawkins, y = quant, label = cld), size = 6, vjust=-1, hjust =-1) +
  scale_fill_brewer(palette = "Blues")
...
C.plot <- data_with_complementary_info %>%
  ...
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_text(data = Tk.C, aes(x = TreeDawkins, y = quant, label = cld), size = 6, vjust=-1, hjust =-1) +
  scale_fill_brewer(palette = "Blues")
...
Phos.plot <- data_with_complementary_info %>%
  ...
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_text(data = Tk.Phos, aes(x = TreeDawkins, y = quant, label = cld), size = 6, vjust=-1, hjust =-1) +
  scale_fill_brewer(palette = "Blues")
...
SD.plot <- data_with_complementary_info %>%
 ...
 geom_boxplot(aes(fill = factor(..middle..)), show.legend = FALSE, alpha=0.6) +
  ...
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_text(data = Tk.SD, aes(x = TreeDawkins, y = quant, label = cld), size = 6, vjust=-1, hjust =-1) +
  scale_fill_brewer(palette = "Blues")
...
testRes$p%>% # Extract p-values associated to the correlation coefficients
  ...
  add_header_above(c("<span style='font-size: 20px; font-weight: bold;'>B. Pairwise table of the associated p-values for generalist</span>" = 9), escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  save_kable("corr_pvalue_generalist.png")
img <- image_read("corr_pvalue_generalist.png")
img_resized <- image_scale(img, "800x600")
image_write(img_resized, "corr_pvalue_resized_generalist.png")
...
testRes_SF = cor.mtest(traits_spe_SF, conf.level = 0.95, exact= FALSE)

res_SF %>% 
  kbl(caption ="", digits =2)  %>%
  add_header_above(c("<span style='font-size: 20px; font-weight: bold;'>C. Pairwise table of the correlation coefficients for SF specialists</span>" = 9), escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Cambria")  %>%
...
testRes_SF$p %>% 
  kbl(caption ="", digits =3)  %>%
  add_header_above(c("<span style='font-size: 20px; font-weight: bold;'>D. Pairwise table of the associated p-values for SF specialists</span>" = 9), escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  save_kable("corr_pvalue_SF_specialists.png")
img <- image_read("corr_pvalue_SF_specialists.png")
img_resized <- image_scale(img, "800x600")
image_write(img_resized, "corr_pvalue_resized_SF_specialists.png")
...
testRes_TF = cor.mtest(traits_spe_TF, conf.level = 0.95, exact= FALSE)

res_TF %>% 
  kbl(caption ="", digits =2)  %>%
  add_header_above(c("<span style='font-size: 20px; font-weight: bold;'>E. Pairwise table of the correlation coefficients for TF specialists</span>" = 9), escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Cambria")  %>%
...
testRes_TF$p %>% 
  kbl(caption ="", digits =3)  %>%
  add_header_above(c("<span style='font-size: 20px; font-weight: bold;'>F. Pairwise table of the associated p-values for TF specialists</span>" = 9), escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  save_kable("corr_pvalue_TF_specialists.png")
img <- image_read("corr_pvalue_TF_specialists.png")
img_resized <- image_scale(img, "800x600")
image_write(img_resized, "corr_pvalue_resized_TF_specialists.png")
...
#Dawkins, height and DBH and crown information
complementary_info <- read.csv("C://Users/marion.boisseaux/Dropbox/Mon PC (Jaboty20)/Documents/METRADICA/METRADICAproject/Dataset/Metradica_all_fieldsheet.csv") %>% select(Code, TreeHeight, TreeDawkins, CrownPosition) #already have DBH

...
Table <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1sLs02kNSBbNmDsepvAU38YdwKZPpcZwiAFM_TRw0y04/edit#gid=1159265517", range = "Abundance") %>% select(Genus, Species, Type, Abund, SF, TF, `P-value`) %>% 
  mutate(Abund = as.numeric(Abund))%>% mutate(SF = as.numeric(SF)) %>% mutate(TF = as.numeric(TF)) %>% mutate(`P-value` = as.numeric(`P-value`)) %>% rename(`p-value` = `P-value`, Abundance = Abund, `SF Habitat` = SF,  `TF Habitat` = TF)
...
Table %>%
  ...
  kable_classic_2(full_width = F) %>% save_kable(file = "IndVal_.png", zoom = 10)
...
```{r Indval_subset}
```{r Table S2: Species chosen for the study according to the Indicator values (Indval) within each habitat in French Guiana. }
Table <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1sLs02kNSBbNmDsepvAU38YdwKZPpcZwiAFM_TRw0y04/edit#gid=1159265517", range = "AbundanceSUBSET") %>% select(Genus, Species, Type, Abund, SF, TF, `P-value`) %>% 
  mutate(Abund = as.numeric(Abund))%>% mutate(SF = as.numeric(SF)) %>% mutate(TF = as.numeric(TF)) %>% mutate(`P-value` = as.numeric(`P-value`)) %>% rename(`p-value` = `P-value`, Abundance = Abund, `SF Habitat` = SF,  `TF Habitat` = TF)
...
Table %>%
  ...
  row_spec(0, bold = TRUE) %>% 
  column_spec(1:2, italic = TRUE) %>%
  kable_classic_2(full_width = F) %>% save_kable(file = "IndVal_subset.png")

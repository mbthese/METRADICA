...
Metradica_log <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1rOl9sZl3A055GTajMcZxrJL_Ozx2bJfM_VEBdcpkD00/edit#gid=1375896520", range = "Metradica") %>% 
  dplyr::select(Code, Genus, Species, Name, Habitat, Type, HabitatType,Forest, Gmin, TLP, LA, SLA, LSWC, MajVLA, K, P, C,N, MidribWidth, SecundaryWidth, DBH, TWI) %>% 
...
Data <- Metradica %>% dplyr::select(-Code, -Genus, -Species, -Habitat, -Type, -TWI) %>% relocate(LSWC, .after = TLP) %>% relocate(MidribWidth, .after = MajVLA) %>% relocate(SecundaryWidth, .after = MidribWidth)%>% relocate(MajVLA, .after = LSWC)
...
```{r}
Data <- Metradica %>% select(-Code, -Genus, -Species, Habitat, Type, -TWI) %>% relocate(LSWC, .after = TLP) %>% relocate(MidribWidth, .after = MajVLA) %>% relocate(SecundaryWidth, .after = MidribWidth)

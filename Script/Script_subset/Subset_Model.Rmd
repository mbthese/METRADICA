...
cv_TF <- data_specialist %>% 
  ...
  mutate(TLP = -TLP)%>%
  group_by(Name) %>% 
  summarise_at(c("Gmin","TLP", "LSWC", "MajVLA", "K", "C", "N", "P"), funs(cv))
...
cv_tot <- bind_rows(cv_long_BF, cv_long_TF) 
...
cv_tot2 <- rbind(cv_BF, cv_TF) %>% filter(Name != "mean")
...
cv_gmin <- cv_tot2 %>% dplyr::select(Name, Gmin, Type) %>%
  ...
  labs(y = bquote(g[min])) +
  scale_fill_manual("Habitat\npreference", 
                    values = c("#56B4E9", "#E79F02"), 
                    labels = c("TF" = "TF specialist",
                               "BF" = "SF specialist")) +
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
 # stat_compare_means(aes(label =                         paste("p=",as.numeric(..p.format..))),method="anova",size=5)
  stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =1.5, label.y =100)
...
summary(res.aov)
kruskal.test(Gmin ~ Type, data = cv_tot2 %>% dplyr::select(Name, Gmin, Type))
...
cv_tlp <- cv_tot2 %>% dplyr::select(Name,TLP, Type) %>%
  ...
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
  stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =1.5, label.y =27)
...
#   theme_minimal() +
#   theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
#     stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =2, label.y =8)
...
cv_MajVLA <- cv_tot2 %>% dplyr::select(Name,MajVLA, Type) %>%
  ...
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
  stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =1.5, label.y =30)
res.aov <- aov(MajVLA ~ Type, data = cv_tot2 %>% dplyr::select(Name,MajVLA, Type) )
plot(res.aov, 1)
plot(res.aov, 2)
summary(res.aov)
kruskal.test(MajVLA ~ Type, data = cv_tot2 %>% dplyr::select(Name,MajVLA, Type) )
...
cv_K <- cv_tot2 %>% dplyr::select(Name,K, Type) %>%
  ...
  theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
   stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =1.5, label.y =30)
  # stat_compare_means(method ="anova", size= 4, label.x =1, label.y =35) +
  # stat_compare_means(comparisons = my_comparaison, hide.ns = TRUE) # Add pairwise comparisons p-value

res.aov <- aov(K ~ Type, data = cv_tot2 %>% dplyr::select(Name,K, Type) )
plot(res.aov, 1)
plot(res.aov, 2)
summary(res.aov) #pvalue <0.05
compare_means(K ~ Type,  data = cv_tot2 %>% dplyr::select(Name,K, Type))
my_comparaison<- list(c("BF", "TF"))
A<- pairwise.t.test(cv_K$K, cv_K$Type,
                 p.adjust.method = "BH") #generalist and BF are different
kruskal.test(K ~ Type, data = cv_tot2 %>% dplyr::select(Name,K, Type))
...
cv_C <- cv_tot2 %>% dplyr::select(Name,C, Type) %>%
  ...
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
  stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =1.5, label.y =2)
...
cv_N <- cv_tot2 %>% dplyr::select(Name,N, Type) %>%
  ...
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
  stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =1.5, label.y =80)
...
cv_LSWC <- cv_tot2 %>% dplyr::select(Name,LSWC, Type)%>%
  ...
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "bottom")+
  stat_compare_means(label = "p.signif", method ="anova", size= 5, label.x =1.5, label.y =6)
...
ggsave(filename = "CV_plots_H3.png", plot = prow_cv, bg = "white", width = 10, height = 8, dpi = 600)
```
...
#variance partitionning for these traits V1
library(ape)

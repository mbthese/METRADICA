...
Metradica_log <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1rOl9sZl3A055GTajMcZxrJL_Ozx2bJfM_VEBdcpkD00/edit#gid=1375896520", range = "Metradica") %>% dplyr::select(Code, Genus, Species, Name, Habitat, Type, HabitatType, Forest, C, TLP, LA, SLA, LSWC, MajVLA, K, P, C, N, MidribWidth, SecundaryWidth, DBH, TWI) %>% 
  mutate_at(c("C","TLP" ,"LA", "SLA", "LSWC", "MajVLA", "MidribWidth", "SecundaryWidth", "K", "P", "C", "N"), abs) %>% 
  mutate_at(c("C","TLP" ,"LA", "SLA", "LSWC", "MajVLA", "MidribWidth", "SecundaryWidth", "TWI","K", "P", "C", "N"), log) %>% 
  mutate(TLP = -TLP) %>% 
  na.omit() 
...
plot(res.aov2, 1)
library(car)
leveneTest(Gmin ~ Habitat, data = data)
...
res.aov2 <- aov(LA ~ Habitat, data = data)
summary(res.aov2)

group_by(data, Habitat) %>%
  summarise(
    count = n(),
    mean = mean(LA, na.rm = TRUE),
    sd = sd(LA, na.rm = TRUE)
  )
...
plot(res.aov2, 1)
library(car)
leveneTest(LA ~ Habitat * Forest, data = data)

data_summary <- group_by(data, Habitat) %>%
  summarise(mean=mean(LA), sd=sd(LA)) %>%
  arrange(desc(mean))
print(data_summary)
...
LA <- ggplot(data, aes(x = Habitat, y = LA)) +
  geom_boxplot()+
#stat_summary(fun.data = 'mean_se', geom = 'errorbar', width = 0.2) +
 # stat_summary(fun.data = 'mean_se', geom = 'pointrange') +
 # geom_point(data=data_summary, aes(x=Habitat, y=mean))+
...
ggsave(path = "./Results/Boxplot/Subset/", filename = "LA_Paracou_HabitatPref.png", plot = LA, bg = "white", width = 8, height = 8, dpi = 600)
...
res.aov2 <- aov(SLA ~ Habitat , data = data)
summary(res.aov2)
...
plot(res.aov2, 1)
library(car)
leveneTest(SLA ~ Habitat * Forest, data = data)

data_summary <- group_by(data, Habitat) %>%
  summarise(mean=mean(SLA), sd=sd(SLA)) %>%
  arrange(desc(mean))
print(data_summary)
...
SLA <- ggplot(data, aes(x = Habitat, y = SLA)) +
  geom_boxplot()+
#stat_summary(fun.data = 'mean_se', geom = 'errorbar', width = 0.2) +
 # stat_summary(fun.data = 'mean_se', geom = 'pointrange') +
 # geom_point(data=data_summary, aes(x=Habitat, y=mean))+
...
ggsave(path = "./Results/Boxplot/Subset/", filename = "SLA_Paracou_HabitatPref.png", plot = SLA, bg = "white", width = 8, height = 8, dpi = 600)
...
Sum <- (Gmin | TLP | LSWC)/(LA | SLA | MajVLA)/(K | P | C | N ) + plot_layout(guides = "collect")

ggsave(path = "./Results/Boxplot/Subset/", filename = "Summary_Paracou.png", plot = Sum, bg = "white", width = 15, height = 8, dpi = 600)

data_Paracou <-  data %>% filter(Forest == 'Paracou')

...
traits <- Data %>% 
  dplyr::select(-Name, -Habitat, -Type, -Forest, -Plot, -TWI, -DBH) #already log
...
#pairwise table correlation coef
coef_corr <- traits %>%
  na.omit() %>%
  cor(method = "spearman") %>%
  kbl(caption ="Pairwise table of the correlation coefficients", digits =2) %>%
  kable_classic(full_width = F, html_font = "Cambria") 
...
# Extract p-values
res$P%>%
  kbl(caption ="Pairwise table of the associated pvalues", digits =3)  %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%

...
```{r}
Paracou_FTH<- read_csv("Clean_data/V1/Paracou_FTH.csv")
```
...
#Stats
model2<- lm(MajVLA~Species, data=veindata)
summary(modele) #il y a clairement un effet espèce

model1 <-lm(MajVLA~1, data=veindata)
summary(model2)
...
D <- veindata %>%
 filter(!(Forest %in% "NA")) %>%
 filter(!(`pref Habitat` %in% "NA") | is.na(`pref Habitat`)) %>%
...
#Stats
model2<- lm(MajVLA~Habitat, data=veindata)
summary(model2) 
...
F
#Stats
model2<- lm(MajVLA~Pref_Habitat, data=veindata)
summary(modele) 
...
H <- ggplot(veindata) +
  aes(x = Forest, y = MajVLA, fill = Forest) +
  geom_boxplot(shape = "circle") +
   scale_fill_manual(
    values = list(
      Bafog = "#F4C71C",
